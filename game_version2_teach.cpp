...
void start_the_game(int id) {   //遊戲運作
    ...
    while(player_now != -1){
        ...
        /*  - for loop
            - 怪獸編號: 1 ~ difficulty
            - 陣列: monster_now[i] 紀錄怪獸當前位置
            - 函式: monster_run(怪獸編號, 怪獸位置, 關卡id) 回傳怪獸移動後位置
            - Hint: monster_run 回傳 -1 代表...?
        */

        //----------
        TODO <2 玩家操作>
        // Hint: "player_now != -1" 在while loop的判斷式中
        ...
        if( scores >= level_up[id] ){
            ...
            TODO <3 選擇一個output函式> // (1) output_graph(int id);  (2) start_output();  (3) died_output();  (4) you_win_output();  (5) thank_you_output();
            SetColor(251);
            cout << "\n\n";
